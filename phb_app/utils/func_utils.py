...
def get_origin_from_file_name(file_name: str,
                              country_data: "CountryData",
                              countries_enum: "CountriesEnum") -> str:
    '''Checks the workbook's origin. Returns the country of origin.'''

    # Pattern in all German timesheets
    german_patterns = country_data.get_locale_by_country(
        countries_enum.GERMANY.value).file_patterns
    # Pattern in all Romanian timesheets
    romanian_patterns = country_data.get_locale_by_country(
        countries_enum.ROMANIA.value).file_patterns
...
def try_load_workbook(workbook: "ManagedWorkbook",
                      wb_type: "ManagedOutputWorkbook",
    ...
    try:
        ...
        raise WorkbookLoadError(
            f"Workbook '{file_name}' is read-only: {str(e)}.") from e
    except InvalidFileException as e:
        raise WorkbookLoadError(
            f"Invalid Excel file '{file_name}': {str(e)}.") from e
    except FileNotFoundError as e:
        raise WorkbookLoadError(
            f"File '{file_name}' not found.") from e
    except PermissionError as e:
        raise WorkbookLoadError(
            f"Permission denied: Close '{file_name}' if open.") from e
    except zipfile.BadZipFile as e:
        raise WorkbookLoadError(
            f"Corrupted Excel file '{file_name}': {str(e)}") from e

def is_file_already_in_table(file_path: str,
                             col: int,
                             *tables: QTableWidget) -> bool:
...
def german_abbr_month(month_number: int,
                      months: dict) -> str:
...
@lru_cache(maxsize=1)
def get_budgeting_dates(file_path: str,
                        sheet_name: str) -> list[tuple[int, int, int]]:
...
# Only cache one result to minimise memory use
@lru_cache(maxsize=1)
def locate_employee_range(
        sheet_obj: Worksheet,
        emp_range: "EmployeeRange",
        anchors: "EmployeeRowAnchors") -> None:
...
def yield_hours_coord(coord: str,
                      row: int) -> Iterator[str]:
...
def find_predicted_hours(emp_dict: dict[str, "Employee"],
                         row: int,
                         file_path: str,
...
def set_selected_project_ids(mngd_wb: "ManagedInputWorkbook",
                             table: QTableWidget,
                             rows: list[QModelIndex],
                             headers: "ProjectIDTableHeaders") -> None:
    '''Sets the selected projects IDs as references from the selectable IDs.'''

    for row in rows:
        # Get the project ID and file name from each row
        proj_id = table.item(
            row.row(), headers.PROJECT_ID.value).text()
        file_name = table.item(
            row.row(), headers.FILENAME.value).text()
        if file_name == mngd_wb.file_name:
            ...
            mngd_wb.managed_sheet_object.selected_project_ids[proj_id] =\
            mngd_wb.managed_sheet_object.selectable_project_ids[proj_id]
